...
export const metadata: Metadata = {
  ...
  description: "Dokumentasi visual kegiatan TEKAD — Tim Media Kreatif Administrasi Bisnis UNM.",
};
...
export default async function GaleriPage() {
  ...
  const getAspectClass = (index: number) => {
    if (index === 0) return "aspect-square md:aspect-auto md:min-h-[400px]";
    if (index === 4) return "aspect-square md:aspect-[2/1]";
    ...
    <>
      ...
      <section className="relative overflow-hidden border-b border-gray-100 bg-gray-950">
        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-14 pt-20 sm:px-6 sm:pb-16 sm:pt-24 lg:px-8">
          <div className="mx-auto max-w-2xl text-center">
            <span className="mb-4 inline-block text-xs font-bold uppercase tracking-[0.2em] text-blue-400">
              Dokumentasi
            </span>
            ...
            <p className="mx-auto mt-4 max-w-xl text-sm text-gray-400 sm:text-base">
              Kumpulan momen dan dokumentasi visual dari berbagai kegiatan TEKAD UNM.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          {photos.length === 0 ? (
            <div className="flex min-h-[300px] flex-col items-center justify-center gap-4 rounded-2xl border border-dashed border-gray-200 bg-gray-50/50">
            ...
            <>
              <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 lg:gap-5">
                {photos.map((foto, index) => (
                  <div
                    key={foto.id}
                    className={`group relative cursor-pointer overflow-hidden rounded-2xl bg-gray-100 ${getSpanClass(index)} ${getAspectClass(index)}`}
                  >
                    {foto.image_url ? (
                      <Image
                        src={foto.image_url}
                    ...
                    <div className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-black/70 via-black/20 to-transparent p-4 opacity-0 transition-all duration-300 group-hover:opacity-100 sm:p-5">
                      <div className="translate-y-3 transition-transform duration-300 group-hover:translate-y-0">
                        {foto.is_featured && (
                          <span className="mb-1.5 inline-block rounded-md bg-amber-500/90 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white">
                            Featured
                          </span>
                        )}
              ...
              <p className="mt-12 text-center text-xs text-gray-400">
                Menampilkan {photos.length} foto dokumentasi kegiatan
              </p>
            </>
          )}
        </div>
